...
class Spider:
    ...
    def LoadPageContent(self, page):
        ...
        # 正则获取
        pattern_link = re.compile(r'<div.*?class="pic-word">.*?<img src="(.*?)".*?'r'<a.*?class="lady-name".*?href="(.*?)".*?>(.*?)</a>.*?'
                                  r'<em>.*?<strong>(.*?)</strong>.*?'r'<span>(.*?)</span>', re.S)
    ...
    def getDetailPage(self, url, name, begin_time):
        ...
        filedir = self.dirName + '/' + name
        self.mkdir(filedir)

        # 保存头像
        try:
            self.saveIcon(icon_url, filedir, name)
        except Exception, e:
        ...
        # 保存个人信息 以及耗时
        try:
            self.saveBrief(brief, filedir, name, end_time - begin_time)
        except Exception, e:
    ...
    def saveIcon(self, url, filedir, name):
        print u'头像地址%s %s ' % (url, name)

        splitpath = url.split('.')
        ftail = splitpath.pop()
        filename = filedir + '/' + name + '.' + ftail
        print filename
    ...
    # 保存个人信息
    def saveBrief(self, content, filedir, name, speed_time):
