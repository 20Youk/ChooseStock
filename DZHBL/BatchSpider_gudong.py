...
class EnterpriseInfoSpider:
    def __init__(self):
        # 文件相关
        self.excelPath = '../../file/person_data.xlsx'
        ...
        # 数据字段名9个
        self.fields = [u'公司名称', u'网站链接', u'法人', u'注册资本', u'成立日期', u'邮箱', u'电话', u'地址', u'备注']
    ...
    def init(self):
        ...
        try:
            ...
            self.personlist = self.worksheet.col_values(0, start_rowx=1)
    ...
    def start_spider(self):
        ...
        # [u'公司名称', u'网站链接', u'法人', u'注册资本', u'成立日期', u'邮箱', u'电话', u'地址', u'备注']
        for item in self.personlist:
            keyword = {"key": item.encode('utf8'), "index": "14", "province": "GD"}
            # response = requests.request("GET", self.catalogUrl, headers=headers, params=keyword)
            # url = (self.catalogUrl % item).encode('utf8')
            # driver.get(url)
            response = requests.get(self.catalogUrl, headers=headers, params=keyword)
