...
while k <= 10:
    ...
    for day in daylist:
        ...
        aWeight = sum([j for j in dict2['Weight']]) / len(dict2['Weight'])
        aList = {a: [count1[a], dict1['MV'][a], aWeight, 0] for a in list(dict1['MV'].keys()) if a not in list(dict2['MV'].keys())}  # a.存在于质量篮子但不存在于HS300篮子,保留质量篮子行业,权重未计算
        ...
        bList = {b: [count1[b], dict2['MV'][b], dict2['Weight'][b], 2] for b in list(dict1['MV'].keys()) if b in list(dict2['MV'].keys())}     # b.质量和HS300交集部分,将HS300行业权重分配给质量
        cList = {c: [count2[c], dict2['MV'][c], dict2['Weight'][c], 1] for c in list(dict2['MV'].keys()) if c not in list(dict1['MV'].keys())}   # c.存在HS300但不存在质量篮子,将HS300行业及权重添加到质量
        aCount = int(sum([l[0] for l in aList.values()]))
        # 整合筛选出来的代码及对应权重 codeDict = {代码: [权重，行业代码，行业名称，是否存在HS300（0：不存在，1：只存在HS300，2：都存在）]}
        codeDict = {}
        for i in range(0, len(data1)):
            if data1[i][4] in bList.keys():
                codeDict[data1[i][2]] = [bList[data1[i][4]][2] / bList[data1[i][4]][0], data1[i][4], data1[i][3], bList[data1[i][4]][3]]
            elif data1[i][4] in aList.keys():
                codeDict[data1[i][2]] = [aWeight / aCount, data1[i][4], data1[i][3], aList[data1[i][4]][3]]
        for m in range(0, len(data2)):
            if data2[m][3] in cList.keys():
                codeDict[data2[m][1]] = [data2[m][5], data2[m][3], data2[m][2], cList[data2[m][3]][3]]
                # codeDict[data2[m][1]] = cList[data2[m][3]][2] / cList[data2[m][3]][0]
        # 计算收益率
        ...
        newData3 = []
        sumWeight = float(sum([codeDict[j][0] for j in codeDict.keys()]))
        for n in range(0, len(data3)):
            newData3.append([data3[n][0], data3[n][1], data3[n][2] * codeDict[data3[n][1]][0]])
        ...
        codeReturn = codeData['PctChg'].groupby(codeData['FDate']).sum() / sumWeight - 0.003 * 2 / 240
        # 查询HS300的每日收益率
        cursor.execute(sqlHS300PctChg, (startday, endDay))
        data4 = cursor.fetchall()
        indexReturn = dict(data4)
        sqldata1.extend(map(lambda x: [startday, x, codeDict[x][0], codeDict[x][1], codeDict[x][2], codeDict[x][3]], codeList))
        dayReturn.extend(map(lambda x: [x, codeReturn[x], indexReturn[x], float(codeReturn[x]) - float(indexReturn[x])], codeReturn.keys()))
        codeCount.append([startday, aCount, int(sum([l[0] for l in bList.values()])), int(sum([l[0] for l in cList.values()]))])
    k += 1
dayReturn.sort(key=lambda x: x[0])
sqldata1.sort(key=lambda x: (x[0], x[3], x[1]))
...
# 将sql执行结果插入excle
workbook = ws.Workbook('C:\Users\Administrator\Desktop\Li_Quality_HS300_%s.xlsx' % today)
sheet1 = workbook.add_worksheet(u'调仓清单')
sheet2 = workbook.add_worksheet(u'收益计算')
field1 = [u'调仓日', u'证券代码', u'股票权重', u'行业代码', u'行业名称', u'是否在HS300']
