                    ...
                    if newData[i][0] not in newCodeList:
                        if int(newData[i][2] * 10000) >= 1000 or int(newData[i][2] * 10000) <= -500 or newData[i][1] == dayData[m][1]:
                            ...
                            if upCount == 0:
                                ...
                                upReturn = upSum / upCount
                            if downCount == 0:
                                downReturn = 0
                            else:
                                downReturn = downSum / downCount
                returnSummary.append([dayData[m][0], dayData[m][1], sum(newReturnList) / len(newReturnList), dayData[m][2], sum(newReturnList) / len(newReturnList) - dayData[m][2], upCount, downCount, keepCount, upReturn, downReturn])
